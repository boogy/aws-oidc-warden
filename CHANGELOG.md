...
- `parseECKey` builds JWKS EC keys through `ecdsa.ParseUncompressedPublicKey` instead of the deprecated `PublicKey.X`/`.Y` fields. Coordinates are left-padded to the curve width, so keys with stripped leading zeros are still accepted.
...
- Tests that only needed transitive session tags switched from the deprecated `tag_auth.transitive_session_tags` to the top-level `session_tags_transitive`. The two rows that assert the deprecated fallback itself keep it.
...
- Two uncovered branches of the tag-auth fallback in `ProcessRequest` now have tests. `TestProcessRequest_TagAuthReadFailureDenies` pins that a failed `GetRoleTags` denies rather than falling through as if the role carried no disqualifying tags, and that the AWS error reaches the log stream — the `tagsErr` field existed on both test consumers but no test ever set it. `TestProcessRequest_TagAuthAuditRecordsMatchedVia` pins `matchedVia: "tag-auth"` on the audit record, which was only ever asserted for `explicit`; it is the only signal distinguishing a grant that came from a role tag from one that came from config. Both were proved to guard: the first fails when the read-error branch sets `allowed = true` and again when its `log.Warn` is dropped, the second when `rec.MatchedVia` is not set.
...
- **`apigw` mode: a `none_of` deny-list could never fire on a list- or object-valued claim, authorizing exactly the caller it was written to refuse.** The API Gateway HTTP API v2 JWT Authorizer hands its verified claims to the Lambda as a `map[string]string`, so every claim whose JSON value is not a scalar arrives already rendered through Go's `%v`: an array `["break-glass"]` becomes the string `"[break-glass]"`, an object becomes `"map[break-glass:true]"`. `mapClaimsFromStrings` special-cased only `aud`, so every other stringified claim reached the condition engine as an ordinary, perfectly readable string — decidable, and matching no pattern the operator had written against the real claim. Under a plain AND that denies, which is fail-closed and invisible. Under `none_of` the direction inverts: a leaf that decidably does not match is a veto that does not fire, so `none_of: [{groups: "break-glass"}]` **granted** the role to a caller carrying that very group. Self mode denied the same logical token — an array is matched element-wise, an object is undecidable and vetoes — so the two validation modes disagreed on the same token, and the safer-looking mode was the one that failed open. Only `jwt_validation.mode: apigw` is affected; `self` and `alb` parse real JSON and were never reachable. A claim whose original JSON type was destroyed by the upstream stringifier is now carried as `types.OpaqueClaim`: its text stays readable, so a positive condition still matches the verbatim string exactly as before, but the value is explicitly undecidable under negation, so a `none_of` veto fires on it rather than being disarmed by it. The two polarities are therefore deliberately **not** complements for this one shape — `NOT(NOT(x)) == x` does not hold for an `OpaqueClaim` — and that is the point: both directions now deny, fail closed in the direction that matters. The cost is precision, and it is worth stating plainly. A scalar claim whose value genuinely is a bracketed string (`"[unclassified]"`, a `map[...]`-shaped free-text value) is indistinguishable from a stringified array once the authorizer has rendered it, so under a `none_of` it now vetoes and **denies** in `apigw` where `self` — which sees a real JSON string — grants. That is a deny-side divergence, chosen over the fail-open one it replaces; use `claims:` on a non-bracketed value, or `self`/`alb` mode, if a bracketed scalar must be gateable under a deny-list. `iss` is exempt (issuer routing needs a plain string), `aud` keeps its existing dedicated decoder, and `exp`/`iat`/`nbf` stay numeric. `sub` is deliberately _not_ exempt: a stringified `sub` is a shape no issuer should mint, and wrapping it makes the canonical subject fail closed downstream instead of authorizing on a rendered array. `TestApigwNoneOfDenyListOpaqueClaimParity` drives the same array- and object-shaped claim through both `self` (with a genuinely signed JWT) and `apigw`, asserts both DENY, and asserts the two modes agree; it fails, reporting the granted role, when either half of the fix is removed.

- **Known limitation — the modes still diverge in the positive direction, and this release does not change that.** "The modes agree" above is about the `none_of` direction only. An allow-list condition on a list-valued claim — `conditions: {groups: "deployers"}` — matches element-wise in `self` and **grants**, while in `apigw` the same claim arrives as the single rendered string `"[deployers other]"`, which no element-wise pattern matches, so it **denies**. That divergence predates this fix, is fail-closed, and is inherent to the API Gateway JWT Authorizer's `map[string]string` claim contract: the element boundaries are gone before the Lambda sees the claim, and any attempt to recover them by re-splitting the rendered text would guess at values containing spaces or brackets. Write allow-lists on list-valued claims against `self`/`alb` mode, or gate on a scalar claim, when running behind an API Gateway JWT Authorizer.

### Fixed

- **An integral claim above 2^53 rendered in scientific notation, so a condition written against its decimal spelling could not match.** JSON numbers decode to `float64`, and `utils.FormatClaimValue` took its exact-integer path only at or below `1<<53`; anything larger fell through to `%v`, which switches to exponent form. A numeric claim such as `uid: 9007199254740994` reached conditions, tag authorization, the audit record, and STS session tags as `"9.007199254740994e+15"`, so `none_of: [{uid: "9007199254740994"}]` never matched and the veto missed — the same fail-open direction as the `apigw` finding above, reachable from any issuer minting large numeric claims. **Every** finite integral `float64` is now rendered with `strconv.FormatFloat(f, 'f', -1, 64)`, at any magnitude: output at or below 2^53 is byte-identical to before, and above it the exact decimal is used. There is no upper cutoff — an earlier form of this fix stopped at `1e21`, where `%v` itself switches to exponent form, which left the same fail-open shape intact for a larger claim (`uid: 1e21` rendered `"1e+21"`, a spelling no operator writes a pattern against, so a `none_of` written in decimal still missed). `±Inf` is excluded explicitly because it satisfies `f == math.Trunc(f)`, and `NaN` with it. The underlying collision is inherent to `float64` and is **not** fixed: two distinct integers above 2^53 can decode to the same `float64` and therefore render identically, so a condition, tag, or session tag on such a claim cannot distinguish them. This is documented on `FormatClaimValue`; gate on a string-valued claim if exact large-integer identity matters.
...
- **Tag-based authorization can constrain any claim, for any issuer: `aow/claim.<name>`.** Every tag suffix `Authorize()` understood — `repo`, `repo-owner`, `branch`, `ref`, `ref-type`, `event-name`, `workflow-ref`, `environment`, `runner-environment`, `actor` — names a GitHub Actions claim, so a non-GitHub issuer could tag-authorize a role on `aow/subject` and nothing else: there was no way to say "this GitLab role additionally requires `project_path: acme/api`". `aow/claim.<name>` reads the raw verified claim `<name>` and ANDs with every other gate, giving tag-auth the same claim-native reach `conditions:` gained this release. It is **not** an identity tag — a role carrying only `aow/claim.*` tags is still denied by the identity gate — so it can narrow access but never grant it, and the issuer and identity gates run first and unchanged. Like a `conditions:` key, a tag is compared against the claim's VALUE rather than its JSON type: a scalar is rendered through `utils.FormatClaimValue` — the same formatter the audit record and the STS session tag use — so `aow/claim.email_verified = true` matches whether the issuer mints the bool `true` or the string `"true"`, and `aow/claim.seats = 42` matches the JSON number. A list-valued claim matches when any element matches, read the same way. Only a shape with no readable text — an object, `null`, or an absent claim — never matches, which is fail-closed here because `aow/claim.*` can only narrow. Reading strings alone would have made this form useless to exactly the issuers it was added for: GitHub mints every claim as a string, so a bool or numeric claim only ever arrives from somewhere else, and a tag naming one would have denied every caller instead of narrowing as written. Unlike a `conditions:` key, the claim name is matched **case-sensitively**: IAM stores tag keys verbatim, so no case is lost on the way in and the exact name the operator wrote is the one that is read. Existing roles are unaffected — a role with no `aow/claim.*` tag behaves exactly as before.
