...
Build the Lambda variants. Non-negotiables: the binary inside a deploy zip must be named `bootstrap` (`provided.al2023` requirement) and the default target is ARM64.
...
3. Verify: `ls -la build/ && file build/bootstrap-*` — report sizes and architectures (expect ARM64).
