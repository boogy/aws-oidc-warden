...
// Tag-based authorization: the Authorize() matcher across every tag dimension,
// the default_org expansion, prefix confusion, the cross-account/transitive
// knobs, and the Validate() warnings that fire when tag-auth can bypass a
// mapping's session policy or role_session_name.
//
// Internal package: several of these reach unexported state (TagAuth.multiIssuer,
// Config.effective). example_config_test.go covers the exported surface.
