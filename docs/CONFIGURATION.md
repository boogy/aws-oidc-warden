...
This only happens when there is **no** configuration source whatsoever. If a config file (or S3 object) *is* present but declares an empty `issuers` list, `Validate()` hard-fails — the seed never masks a misconfigured file.
...
| `AOW_AUDIT_REQUIRED`     | `audit_required`   | Make the audit-record write a hard, fail-closed dependency: an allow decision only returns credentials after the audit record is durably written; requires `log_to_s3=true` + `log_bucket` set. | `false` |
...
`config_fragments` lists additional sources merged on top of the base config's `default_issuer`, `role_sets`, `role_mappings`, and `role_groups` (and *only* those four keys; anything else in a fragment is a hard error). This lets teams own their own role-mapping fragment without touching the base config that defines `issuers`/hardening knobs/`tag_auth`.
