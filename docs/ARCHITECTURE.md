...
- Leaf matching is polarity-aware. A regex can only be matched against a string, so in positive polarity a bool, number, object, or non-string list element simply fails to match — which denies, and is unchanged. Under an odd number of `none_of` groups that same non-match would disarm the veto and authorize, so the leaf is re-decided on the claim's **value**: a scalar is compared through its canonical text (the same rendering used for audit records and session tags), meaning `none_of: [{email_verified: "false"}]` vetoes the JSON bool `false` and does not veto `true`. Only a shape with no readable text — an object, or a list carrying one — counts as *satisfied* so the veto fires. Absence keeps `none_of`'s exact-negation meaning and is not affected.
